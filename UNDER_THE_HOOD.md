...
- **`server/`**: API (Fastify) + pipeline de ingestão (LangChain) + integrações (Chroma + Ollama).
...
1. O cliente faz `POST /chat` com `{ question }`.
2. O server faz retrieval no Chroma via `similaritySearchWithScore(question, topK)`.
3. Aplica filtros de distância:
...
4. Deduplica chunks recuperados por conteúdo (evita repetir o mesmo trecho no prompt).
5. Monta o prompt com contexto + pergunta e faz **stream** do modelo do Ollama.
6. O endpoint responde em **NDJSON** (`application/x-ndjson`), emitindo eventos do tipo:
...
O `server/src/config.ts` valida as variáveis via `zod` (falha cedo se estiverem faltando/inválidas).
...
- **Prompt estourando contexto**: reduza `CHAT_TOP_K`, aumente chunking (menor `chunkSize`) ou dedupe mais agressivo.
